...
--  $Header: XXSC_EMP_PKG.pls 12.1 2015/11/29 10:30:06 mrahman noship $
--*************************************************************************
-- Copyright (c)  2016   Shuaa Capital PSC
-- All rights reserved
--*************************************************************************
       ...
       --   AND REPLACE(TRANSLATE(phone_number,'-+()',' '),' ', '') =L_EMP_PHONE_NUMBER; 
            ...
            IF p_emp_phone_number IS NOT NULL THEN ---PHONE NUMBER START
              ...
              BEGIN
                ...
                -- -----------------------------
                P_DATE_FROM => sysdate, P_DATE_TO => fnd_conc_date.string_to_date('31-DEC-4712'), P_PHONE_TYPE => 'M', P_PHONE_NUMBER => P_EMP_PHONE_NUMBER, P_PARENT_ID => LN_PERSON_ID, P_PARENT_TABLE => 'PER_ALL_PEOPLE_F', p_effective_date => SYSDATE+1,
