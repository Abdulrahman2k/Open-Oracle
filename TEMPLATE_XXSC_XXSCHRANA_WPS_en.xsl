...
<!-- $Header: TEMPLATE_XXSC_XXSCHRANA_WPS_en.xsl 115.0 2016/05/23 09:25:39 xdouser noship $ -->
<!-- dbdrv: none -->
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:ora="http://www.oracle.com/XSL/Transform/java/" xmlns:xdofo="http://xmlns.oracle.com/oxp/fo/extensions" xmlns:xdoxslt="http://www.oracle.com/XSL/Transform/java/oracle.apps.xdo.template.rtf.XSLTFunctions" xmlns:xdoxliff="urn:oasis:names:tc:xliff:document:1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:OutBoundPayments="http://xmlns.oracle.com/iby/EFTTemplateSample" xmlns:General="http://xmlns.oracle.com/iby" xmlns:Address="http://xmlns.oracle.com/iby/address" xmlns:PaymentsCommon="http://xmlns.oracle.com/iby/PaymentsCommon" xmlns:xdoeft="http://xmlns.oracle.com/iby/xdoeft">
   ...
   <xsl:template match="/">
      ...
      <xsl:for-each select=".//G_EDR" xdoeft:type="LEVEL" xdoeft:level-name="G_EDR">
         ...
         <xsl:call-template name="L1R1P24S6"/>
         <xsl:value-of select="xdoxslt:append_to($_XDOCTX, 'L1R1', xdoxslt:get_variable($_XDOCTX, 'L1R1P24S6'))"/>
         <!--Render: -->
      ...
      <!--Control: ================ Start new level: XXSCHRANA_WPS=================-->
      <xsl:for-each select=".//XXSCHRANA_WPS" xdoeft:type="LEVEL" xdoeft:level-name="XXSCHRANA_WPS">
         <!--Render: <COMMENTS>-->
         ...
         <xsl:call-template name="L1R2P24S38"/>
         <xsl:value-of select="xdoxslt:append_to($_XDOCTX, 'L1R2', xdoxslt:get_variable($_XDOCTX, 'L1R2P24S38'))"/>
   ...
   <xsl:template name="L1R1P24S6">
      ...
      <xsl:choose>
         <xsl:when test="string-length(string($v20))>24">
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v21', substring(string($v20), 1,24))"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v21', $v20)"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R1P24S6', xdoxslt:get_variable($_XDOCTX, '$v21'))"/>
   </xsl:template>
   ...
   <xsl:template name="L1R1P15S14">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_NET)),'NONE')-->
      <xsl:variable name="v42" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_NET))"/>
   ...
   <xsl:template name="L1R1P15S16">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_INCENTIVES)),'NONE')-->
      <xsl:variable name="v48" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_INCENTIVES))"/>
   ...
   <xsl:template name="L1R1P4S18">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: TO_NUMBER((ZERO))-->
      <xsl:variable name="v54" select="xdoxslt:to_number(xdoxslt:normalize_string($_XDOCTX, .//ZERO))"/>
      <xsl:choose>
   ...
   <xsl:template name="L1R2P3S20">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_SCR)),'NONE')-->
      <xsl:variable name="v56" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_SCR))"/>
   ...
   <xsl:template name="L1R2P13S22">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_EMPLOYEER_ID)),'NONE')-->
      <xsl:variable name="v62" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_EMPLOYEER_ID))"/>
   ...
   <xsl:template name="L1R2P15S34">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_TOTALSAL)),'NONE')-->
      <xsl:variable name="v97" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_TOTALSAL))"/>
      <xsl:variable name="v98" select="xdoxslt:convert_case(string($v97), string('NONE'))"/>
      <xsl:choose>
         <xsl:when test="string-length(string($v98))>15">
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v99', substring(string($v98), 1,15))"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v99', $v98)"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R2P15S34', xdoxslt:get_variable($_XDOCTX, '$v99'))"/>
   </xsl:template>
   <xsl:template name="L1R2P1S35">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((',')),'NONE')-->
      <xsl:variable name="v100" select="string(',')"/>
      <xsl:variable name="v101" select="xdoxslt:convert_case(string($v100), string('NONE'))"/>
      <xsl:choose>
         <xsl:when test="string-length(string($v101))>1">
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v102', substring(string($v101), 1,1))"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v102', $v101)"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R2P1S35', xdoxslt:get_variable($_XDOCTX, '$v102'))"/>
   </xsl:template>
   <xsl:template name="L1R2P3S36">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_AED)),'NONE')-->
      <xsl:variable name="v103" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_AED))"/>
      <xsl:variable name="v104" select="xdoxslt:convert_case(string($v103), string('NONE'))"/>
      <xsl:choose>
         <xsl:when test="string-length(string($v104))>3">
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v105', substring(string($v104), 1,3))"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v105', $v104)"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R2P3S36', xdoxslt:get_variable($_XDOCTX, '$v105'))"/>
   </xsl:template>
   <xsl:template name="L1R2P1S37">
      ...
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R2P1S37', xdoxslt:get_variable($_XDOCTX, '$v108'))"/>
   </xsl:template>
   <xsl:template name="L1R2P24S38">
      <xsl:param name="index" select="string(position())"/>
      <!--Expression: CONVERT_CASE(TO_CHAR((CF_IBAN)),'NONE')-->
      <xsl:variable name="v109" select="string(xdoxslt:normalize_string($_XDOCTX, .//CF_IBAN))"/>
      <xsl:variable name="v110" select="xdoxslt:convert_case(string($v109), string('NONE'))"/>
      <xsl:choose>
         <xsl:when test="string-length(string($v110))>24">
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v111', substring(string($v110), 1,24))"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="xdoxslt:set_variable($_XDOCTX,'$v111', $v110)"/>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="xdoxslt:set_variable($_XDOCTX, 'L1R2P24S38', xdoxslt:get_variable($_XDOCTX, '$v111'))"/>
   </xsl:template>
